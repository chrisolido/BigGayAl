<?php
	...
	*/ 
	...
	function post_readtime_install() {
		...
		add_option('post_readtime_prefix', 'Reading time: ', '', 'yes'); // Add the option for prefix to string
  ...
  <?php
	...
	function post_read_time() {
		...
		if($time == "1") {
			...
			$estimated_time = $estimated_time.' '.$minutes . ' min'. ($minutes == 1 ? '' : 's') . ', ' . $seconds . ' sec' . ($seconds == 1 ? '' : 's');		
		}
		if($minutes < 1) {
